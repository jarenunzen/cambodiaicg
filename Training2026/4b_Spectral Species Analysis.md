...
### Step 6
Search your directory for this analysis for the PCA output file. 

> [!WARNING]
> **If you rerun the PCA with a different file OR number of principle compoenents, you will need to adjust this first function.**

```
# Locate and read the PCA raster generated by perform_PCA
# (biodivMapR names this file based on the input name + '_PCA')

print("Searching for the generated PCA raster file...")

# Search recursively for any file containing "PCA" in its name within the output directory
...
# Filter out auxiliary text, log, or header files (.hdr, .txt, .xml)
# We want the actual heavy raster file (.tif, .tiff, .bil, or extensionless data files)
raster_pca_files <- pca_files[!grepl("\\.(hdr|txt|xml|html)$", pca_files, ignore.case = TRUE)]

if (length(raster_pca_files) == 0) {
  stop("CRITICAL: Could not find any PCA raster files in Output_Dir. Check if perform_PCA completed successfully without hidden errors.")
}

# Select the first matching valid raster file found
pca_file_path <- raster_pca_files[1]
print(paste("Success! Found PCA file at:", pca_file_path))

# Load the raster into terra
pc_raster <- rast(pca_file_path)

# Ensure the raster layer names are clean for downstream subsetting
print(pc_raster)

```

### Step 7
Sub-sample the PCA image to build the data for the clustering algorithm. NULL / NA values will be ignored. 

> [!NOTE]
> There is a masking and preprocessing function built into the biodivMapR package to use before this step. **We have performed the preprocessing on the imagery before using it in this tutorial**. 

```

# Extract cell values for clustering (ignoring NA/background space)
print("Extracting pixel data for K-means...")

v <- values(pc_raster, dataframe = TRUE)
v_clean <- na.omit(v)


# Sub-sample to 100,000 pixels (per your script parameters) for efficient training
set.seed(42) # Ensures reproducibility 
sample_idx <- sample(1:nrow(v_clean), min(100000, nrow(v_clean)))
samples <- v_clean[sample_idx, ]
```
